  ...
  <FactoryProvider<Sequelize>>{
    ...
    useFactory: async (configService: ConfigService) => {
      const sequelize = new Sequelize({
        ...
        logQueryParameters: configService.get('NODE_ENV') === 'development',
        logging: (query, time) => {
          Logger.log(time + 'ms' + ' ' + query);
        },
        ...(configService.get('NODE_ENV') === 'production' ? { logging: false } : {}),
        benchmark: configService.get('NODE_ENV') === 'development',
